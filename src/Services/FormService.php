...
class FormService {
    ...
    public function open(array $data = [])
    {
        ...
        if ( $this->_param === true ){
            ...
            $p[] = $this->hidden('hedef', empty(request('hedef')) ? 'ajaxPageContainer' : request('hedef') );
            $p[] = $this->hidden('hedef2', request('hedef2') );
    ...
    /**
     *  Form'a MultiSelect ekler...
     *
     *  @param string $name Name
     *  @param string $label Label
     *  @param string $listLabel Liste başlığını etiketi set edilir.
     *  @param array $value Varsayılan boş array'dır. Değerler set edilir.
     *  @param array $list Varsayılan boştur. Option listesi set edilir. Örnek: array(array('id'=>1, 'value'=>'Ornek'))
     *  @param array $data Varsayılan boştur. Keyler: <br>lid -> array() option id keyi<br>lvalue -> array() option value keyi<br>id<br>class<br>disabled<br>readonly<br>labeltitle<br>desc<br>ulclass
     *  @return none Değer döndürmez.
     */
    /*
    public function selectMulti($name, $label, $listLabel, $value=array(), $list=array(), $data=array())
    {
        //dd($list);

        $p=array();
        $p[]='<select set_multiselect="1"';
        $p[]='multiple="multiple"';
        $p[]='data-name="'.$name.'"';

        if ( !empty($data['id']) ){ $p[]='id="'.$data['id'].'"'; }
        //if ( !empty($name) ){ $p[]='name="'.$name.'"'; }
        if ( !empty($listLabel) ){ $p[]='data-label="'.$listLabel.'"'; }

        if ( !empty($data['class']) ){ $p[]='class="form-control input-sm '.$data['class'].'"'; }else{ $p[]='class="form-control input-sm"'; }
        if ( !empty($data['disabled']) ){ $p[]='disabled="disabled"'; }
        if ( !empty($data['ulclass']) ){ $p[]='data-ulclass="'.$data['ulclass'].'"'; }
        if ( !empty($data['readonly']) ){ $p[]='readonly="readonly"'; }
        ...
        foreach($list as $li){
            $options.='<option value="'.$li[$lid].'"';
            // Value varsa selected
            if ( in_array(@$li[$lid], $value) ){ $options.=' selected="selected"'; }
            if ( isset($li['icon'], $li['icon-color']) ){ $options.=' data-icon="'.$li['icon'].'" data-icon-color="'.$li['icon-color'].'"'; }
            $options.='>'.$li[$lival].'</option>';
        }
        $p[]='>'.$options.'</select>';
        if ($this->_only){ return implode(' ', $p); }
    ...
    public function addInputCheckbox($name, $label, $value='', $checked=false, $data=array())
    {
        ...
        if ( !empty($data['disabled']) ){ $p[]='disabled="disabled"'; }
        if ( !empty($data['readonly']) ){ $p[]='readonly="readonly"'; }
        if ( !empty($data['maxlength']) ){ $p[]='maxlength="'.$maxlength.'"'; }
        $p[]='autocomplete="off"';
        $p[]='/>';
    ...
    public function inputGroup($type, $elementName = null, $optionId = null)
    {
        if ($type == 'account'){
            return (new InputGroup)->account($this->id());

        }elseif ($type == 'contact') {
            return (new InputGroup)->contact($this->id());

        }elseif ($type == 'optionItem'){
            return (new InputGroup)->optionItem($this->id(), $optionId, $elementName);

        }elseif ($type == 'device'){
            return (new InputGroup)->device($this->id());

        }elseif ($type == 'brand'){
            return (new InputGroup)->brand($this->id());

        }elseif ($type == 'brandModel'){
            return (new InputGroup)->brandModel($this->id());
        }
    }
